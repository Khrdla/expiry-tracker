...
async def generate_enhanced_return_form_pdf(return_form: dict):
    ...
    try:
        ...
        doc = SimpleDocTemplate(
            ...
            rightMargin=0.5*cm,   # 5mm - minimal as requested
            allowSplitting=0,     # Disable page breaks completely
            showBoundary=0        # Clean layout
        )
        ...
        ref_table = Table(ref_data, colWidths=[2.2*inch, 4.3*inch])  # Optimized widths
...
async def export_return_form_pdf(
    ...
    try:
        ...
        # Professional currency display with clean formatting and FOC indication
        currency_data = [
            ['Total Value:', f"{total_supplier_currency:.2f} {purchase_currency}" + (' (FREE - No Cost)' if is_foc else '')]
        ]
    ...
    try:
        ...
        try:
            ...
            if total_usd > 0:
                ...
                pdf.cell(0, 6, sanitize_text(f'Total USD Value: ${total_usd:,.2f}'), 0, 1)
            
            pdf.ln(10)
            
            # Summary statistics
            pdf.set_font('Arial', 'B', 12)
            pdf.cell(0, 8, sanitize_text('Summary Statistics:'), 0, 1)
            pdf.set_font('Arial', '', 10)
            
            pdf.cell(0, 6, sanitize_text(f'Total Entries: {report_data.get("total_entries", 0)}'), 0, 1)
            pdf.cell(0, 6, sanitize_text(f'Total Quantity Wasted: {report_data.get("total_quantity_wasted", 0)}'), 0, 1)
            pdf.cell(0, 6, sanitize_text(f'Report Generated: {current_time}'), 0, 1)
            pdf.cell(0, 6, sanitize_text('Status: Complete'), 0, 1)
            
            # Footer
            pdf.ln(20)
            pdf.set_font('Arial', 'I', 8)
            pdf.cell(0, 4, sanitize_text('Generated by GEANT HYPERMARKET Inventory Management System'), 0, 0, 'C')
...
async def export_inventory_scans_pdf(current_user: User = Depends(get_admin_user)):
    ...
    try:
        ...
        for zone_idx, (zone_key, zone_scans) in enumerate(zones_data.items()):
            ...
            # Add zone summary at bottom - one line below table, bold, in Primary Green
            story.append(Spacer(1, 15))
            ...
            # Add footer only on the last page to avoid repetition
            if zone_idx == len(zones_data) - 1:
                story.append(Spacer(1, 30))
                
                # Footer line: thin red line with text
                footer_line_table = Table([['']], colWidths=[6.5*inch])
                footer_line_table.setStyle(TableStyle([
                    ('LINEABOVE', (0, 0), (-1, -1), 1, GEANT_ACCENT_RED),
                    ('TOPPADDING', (0, 0), (-1, -1), 5),
